...
async function scrapeNews(config: ScraperConfig): Promise<NewsItem[]> {
  ...
  const fs = require('fs');
  fs.appendFileSync('scrape-debug.log', `Starting scrape for ${config.name} (${config.url})\n`);
  try {
    ...
    const fs = require('fs');
    fs.appendFileSync('scrape-debug.log', `Fetched ${config.name}: ${html.length} bytes\n`);
    ...
    fs.appendFileSync('scrape-debug.log', `Found ${elements.length} elements for selector "${config.articleSelector}" in ${config.name}\n`);
    ...
    const fs = require('fs');
    fs.appendFileSync('scrape-error.log', `Error scraping ${config.name} (${config.url}): ${error}\n`);
