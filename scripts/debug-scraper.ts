...
async function scrapeNews(config: ScraperConfig) {
  ...
  try {
    ...
    const selectors = [config.articleSelector, 'article', 'h2 a', '.story-card', 'a'];
    ...
    $('h2').each((i, el) => {
        ...
        console.log(`H2 [${i}]: parent=${parent.prop('tagName')} parent_class="${parent.attr('class')}" link_href="${link.attr('href')}"`);
    });
